const {Client,  REST, Routes, GatewayIntentBits, SlashCommandBuilder, IntentsBitField, EmbedBuilder} = require("discord.js")
const {readdirSync, readFileSync, writeFileSync, existsSync} = require('fs');
const auth = JSON.parse(readFileSync('.auth'))
var conf = null
let latestActivityID = existsSync('.latestActivityID') ? readFileSync('.latestActivityID') : 0
...
const loadBot = () => {
    conf = JSON.parse(readFileSync('conf.json'))
    ...
    // Handle the bot
    bot = new Client({ intents: [GatewayIntentBits.Guilds, GatewayIntentBits.GuildVoiceStates, IntentsBitField.Flags.GuildMessages, GatewayIntentBits.MessageContent] });
    ...
    bot.on('ready', async () => {
        ...
        let guild = bot.guilds.resolve(conf.serverID)
        let channel = bot.channels.resolve(conf.channelID)
        if (!guild) {
        ...
        */
        ...
        console.log(`== Bot logged in as @${bot.user.username}. Ready for action! ==`)
        events.start()
    })
    ...
    bot.on('interactionCreate', async interaction => {
        ...
        names.forEach(name => {
            ...
            if (interaction.commandName === name.replaceAll('.js', "")) runCommand(command, bot, interaction);
        })
    })
    ...
    {
        //Checks if you have the right permissions: 0 = Everyone, 1 = Server Insights Permissions,
        // 2 = Server Admins, 3 = Bot Owner
    ...
    */
    ...
    events.on('createCard', async (event, board) => {
        if (!eventEnabled(`cardCreated`)) return
        let embed = (await getEmbedBase(event))
        ...
        send(await addDiscordUserData(embed, event.memberCreator))
    })
    ...
    events.on('deleteCard', async (event, board) => {
    ...
    events.on('commentCard', async (event, board) => {
        let embed = (await getEmbedBase(event))
    ...
    events.on('addMemberToCard', async (event, board) => {
        let embed = (await getEmbedBase(event))
        ...
        let editedEmbed = await addDiscordUserData(embed, event.member)

        if (event.member.id === event.memberCreator.id) {
            if (!eventEnabled(`memberAddedToCardBySelf`)) return
            editedEmbed.setDescription(editedEmbed.data.description + ` added themselves to card.`)
            send(editedEmbed)
        } else {
            if (!eventEnabled(`memberAddedToCard`)) return
            editedEmbed.setDescription(editedEmbed.data.description + ` added to card by **[${conf.realNames ? event.memberCreator.fullName : event.memberCreator.username}](https://trello.com/${event.memberCreator.username})**`)
            send(await addDiscordUserData(editedEmbed, event.memberCreator))
        }
    })

    // Fired when a member is removed from a card
    events.on('removeMemberFromCard', async (event, board) => {
        let embed = (await getEmbedBase(event))
        ...
        if (event.member.id === event.memberCreator.id) {
            ...
            editedEmbed.setDescription(editedEmbed.data.description + ` removed themselves from card.`)
            send(editedEmbed)
        } else {
            if (!eventEnabled(`memberRemovedFromCard`)) return
            editedEmbed.setDescription(editedEmbed.data.description + ` removed from card by **[${conf.realNames ? event.memberCreator.fullName : event.memberCreator.username}](https://trello.com/${event.memberCreator.username})**`)
            send(await addDiscordUserData(editedEmbed, event.memberCreator))
        }
    })

    // Fired when a list is created
    events.on('createList', async (event, board) => {
    ...
    events.on('updateList', async (event, board) => {
        let embed = (await getEmbedBase(event))
    ...
    events.on('addAttachmentToCard', async (event, board) => {
    ...
    events.on('deleteAttachmentFromCard', async (event, board) => {
    ...
    events.on('addChecklistToCard', async (event, board) => {
    ...
    events.on('removeChecklistFromCard', async (event, board) => {
    ...
    events.on('updateCheckItemStateOnCard', async (event, board) => {
    ...
    */
    events.on('maxId', (id) => {
        ...
        latestActivityID = id
        writeFileSync('.latestActivityID', new String(id).valueOf())
    })

    const send = (embed, content = ``) => conf.channel.send({content: `${content} ${conf.contentString}`, embeds: [embed]}).catch(err => console.error(err))
    ...
    const getEmbedBase = async (event) => {
        const botMember = await conf.guild.members.fetch(bot.user.id)
        return new EmbedBuilder()
            .setFooter({text: `${botMember.displayName} • ${event.data.board.name} [${event.data.board.shortLink}]`, iconURL: bot.user.avatarURL()})
            .setTimestamp(event.hasOwnProperty(`date`) ? new Date(event.date) : new Date())
            .setColor("#127ABD")
    }
    ...
    const addDiscordUserData = async (embed, member) => {
        if (conf.userIDs[member.username]) {
            let discordUser = await conf.guild.members.fetch(conf.userIDs[member.username])
            if (discordUser) embed
                .setThumbnail(discordUser.user.avatarURL())
                .setDescription(`${embed.data.description} / ${discordUser.toString()}`)
        }
        return embed
    }
...
exports.reloadConf = output.reloadConf;
exports.reloadBot = output.reloadBot;
